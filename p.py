...
#         book_id = int(self.table.item(selected, 0).text())  # Convert to int
#         title = self.title_input.text()
#         author = self.author_input.text()
#         year = self.year_input.text()
#         genre = self.genre_input.text()
...
from PyQt5.QtWidgets import (
    ...
    QSpinBox, QDateEdit, QComboBox, QMessageBox
)
...
class Library:
    ...
    def add_book(self, title, author, year, genre):
        if not title or not author:  # Basic validation
...
class LibraryManagementApp(QMainWindow):
    ...
    def update_book(self):
        ...
        dialog = BookDialog(self, title, author, year, genre, available)
        if dialog.exec_():
            new_title, new_author, new_year, new_genre, new_available = dialog.get_values()
            try:
                self.library.update_book(book_id, new_title, new_author, new_year, new_genre, new_available)
                self.load_books()
            except ValueError as e:
                self.show_error(str(e))
...
# Dialog for adding or updating books
class BookDialog(QDialog):
    def __init__(self, parent=None, title='', author='', year=2024, genre='', available=True):
        ...
        self.genre_edit = QLineEdit(genre)
        self.available_edit = QComboBox()
        self.available_edit.addItems(["Yes", "No"])
        self.available_edit.setCurrentIndex(0 if available else 1)

        self.layout.addRow("Title:", self.title_edit)
        self.layout.addRow("Author:", self.author_edit)
        self.layout.addRow("Year:", self.year_edit)
        self.layout.addRow("Genre:", self.genre_edit)
        self.layout.addRow("Available:", self.available_edit)
    ...
    def get_values(self):
        return (self.title_edit.text(), self.author_edit.text(), self.year_edit.value(), self.genre_edit.text(), self.available_edit.currentIndex() == 0)
